        ...
        <div className={`flex justify-between ${(hasShadow ? 'h-[70px]' : isHomePage ? 'h-[106px]' : 'h-[70px]')}`}>
          <div className="flex items-center">
            <Link href="/" className={`flex-shrink-0 flex items-center text-yellow-1 ${(hasShadow) ? 'text-yellow-1' : isHomePage ? 'text-white!' : ''}`}>
              <svg className={`${(pathname === 'investment-development' || pathname === 'real-estate-services' || pathname === 'management-operation') ? "text-yellow-1 " : ""} ${(hasShadow) ? 'h-[60px]' : 'h-[72px]'}`} width="60" height="60" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
